...
keys<-spss.get("N:/durable/Raw/Valid_consent/PDB2535_SV_INFO_V12_20260522.sav",
               use.value.labels=FALSE,to.data.frame=TRUE,allow="_")
...
names(dat_mom)<-c("preg_id_2535","m_id_2535","agemax_mom","bmimax_mom","eduyearsmax_mom","smkinitmax_mom","paritymax_mom",
...
names(dat_dad)<-c("preg_id_2535","f_id_2535","agemax_dad","bmimax_dad","eduyearsmax_dad","smkinitmax_dad","paritymax_dad",
...
# DASH QUINTILE FUNCTION #

dash_quintile<-function(x,id){
  q<-quantile(x,probs=seq(0,1,0.2),na.rm=TRUE)
  if(length(unique(q))==6){
    cut(x,q,include.lowest=TRUE,labels=FALSE)
  }else{
    out<-rep(NA_integer_,length(x))
    ok<-!is.na(x) & !is.na(id)
    r<-order(order(x[ok],id[ok]))
    out[ok]<-ceiling(r/length(r)*5)
    out
  }
}
...
# Up to 200 g/day fruit juice contributes proportionally to a maximum
# of 100 g/day to the fruit component. Juice >200 g/day is assigned
# to the beverage component.
food$juice_total_mom<-with(food,spis171+spis172)
food$juice_fruit_mom<-with(food,pmin(juice_total_mom,200)*(100/200))
food$juice_excess_mom<-with(food,pmax(juice_total_mom-200,0))

food$fruit_whole_mom<-with(food,spis148+spis149+spis150+spis153+spis154+spis155+spis156+spis157+spis158+spis159+spis160+spis161+spis162
                           +spis163+spis174+spis175)
food$fruit_mom<-with(food,fruit_whole_mom+juice_fruit_mom)
...
food$juice_mom<-food$juice_excess_mom
...
# Complete information for all 8 DASH components
...
# DASH quintile scores: 1-5
food$dashq_fruit_mom<-dash_quintile(food$fruit_mom,food$preg_id_2535)
food$dashq_veg_mom<-dash_quintile(food$veg_mom,food$preg_id_2535)
food$dashq_nutleg_mom<-dash_quintile(food$nutleg_mom,food$preg_id_2535)
food$dashq_dairy_lf_mom<-dash_quintile(food$dairy_lf_mom,food$preg_id_2535)
food$dashq_wholeg_mom<-dash_quintile(food$wholeg_mom,food$preg_id_2535)
food$dashq_snacks_mom<-dash_quintile(food$snack_proxy_mom,food$preg_id_2535)
food$dashq_sbev_mom<-dash_quintile(food$sbev_mom,food$preg_id_2535)
food$dashq_rpmeat_mom<-dash_quintile(food$rpmeat_mom,food$preg_id_2535)

# Reverse scoring of unfavorable DASH components
vars<-c("dashq_snacks_mom","dashq_sbev_mom","dashq_rpmeat_mom")
for(i in 1:length(vars))
{
  food[,vars[i]]<-6-food[,vars[i]]
}

# Total DASH score: theoretical range 8-40
...
dad$juice_total_g<-with(dad,ff419*250)
dad$juice_fruit_g<-with(dad,pmin(juice_total_g,100))
dad$juice_excess_g<-with(dad,pmax(juice_total_g-100,0))
dad$fruit_dad<-with(dad,(ff442*150)+juice_fruit_g)
...
dad$sbev_dad<-with(dad,juice_excess_g+(ff420*250)+(ff421*250)+(ff422*250)+(ff423*250))
...
dad$dashq_fruit_dad<-dash_quintile(dad$fruit_dad,dad$preg_id_2535)
dad$dashq_veg_dad<-dash_quintile(dad$veg_dad,dad$preg_id_2535)
dad$dashq_nutleg_dad<-dash_quintile(dad$nutleg_dad,dad$preg_id_2535)
dad$dashq_dairy_lf_dad<-dash_quintile(dad$dairy_lf_dad,dad$preg_id_2535)
dad$dashq_wholeg_dad<-dash_quintile(dad$wholeg_dad,dad$preg_id_2535)
dad$dashq_snacks_dad<-dash_quintile(dad$snack_proxy_dad,dad$preg_id_2535)
dad$dashq_sbev_dad<-dash_quintile(dad$sbev_dad,dad$preg_id_2535)
dad$dashq_rpmeat_dad<-dash_quintile(dad$rpmeat_dad,dad$preg_id_2535)

vars<-c("dashq_snacks_dad","dashq_sbev_dad","dashq_rpmeat_dad")
for(i in 1:length(vars))
{
  dad[,vars[i]]<-6-dad[,vars[i]]
}
...
# Count unique persons AFTER applying each criterion. This avoids undercounting people
# whose first pregnancy does not meet a criterion but a later pregnancy does.
n_unique<-function(x) length(unique(x[!is.na(x)]))

# Number of unique valid MoBa pregnancies with valid consent
n_preg<-length(unique(dat$preg_id_2535[!is.na(dat$preg_id_2535)]))

flow<-data.frame(
  step=c("Valid MoBa pregnancies with valid consent",
         "Total unique participants",
         "Planned pregnancy",
         "Planned + infertility information",
         "Planned + infertility + own dietary information",
         "Planned + infertility + dietary information in both parents"),
  pregnancies=c(
    n_preg,
    NA,
    NA,
  ...
  mothers=c(
    NA,
    n_unique(dat$m_id_2535),
    n_unique(dat$m_id_2535[dat$preg_plan==1]),
    n_unique(dat$m_id_2535[dat$preg_plan==1 & !is.na(dat$subf)]),
    n_unique(dat$m_id_2535[dat$preg_plan==1 & !is.na(dat$subf) & dat$diet_mom==1]),
    n_unique(dat$m_id_2535[dat$preg_plan==1 & !is.na(dat$subf) & dat$diet_mom==1 & dat$diet_dad==1 & !is.na(dat$f_id_2535)])
  ),
  fathers=c(
    NA,
    n_unique(dat$f_id_2535),
    n_unique(dat$f_id_2535[dat$preg_plan==1]),
    n_unique(dat$f_id_2535[dat$preg_plan==1 & !is.na(dat$subf)]),
    n_unique(dat$f_id_2535[dat$preg_plan==1 & !is.na(dat$subf) & dat$diet_dad==1]),
    n_unique(dat$f_id_2535[dat$preg_plan==1 & !is.na(dat$subf) & dat$diet_dad==1 & dat$diet_mom==1 & !is.na(dat$m_id_2535)])
  )
)

print(flow)
write.csv(flow,"./Outputs/Results/flow_chart_counts.csv",row.names=FALSE)
